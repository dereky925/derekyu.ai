const HILLS = [
  { x: 0.16, y: 0.28, a: 1.15, s: 22, p: 0.4 },
  { x: 0.58, y: 0.2, a: 1.05, s: 18, p: 1.3 },
  { x: 0.82, y: 0.42, a: 1.2, s: 20, p: 2.1 },
  { x: 0.34, y: 0.52, a: 0.9, s: 16, p: 0.8 },
  { x: 0.7, y: 0.68, a: 0.95, s: 19, p: 2.7 },
  { x: 0.12, y: 0.74, a: 0.85, s: 17, p: 1.9 },
  { x: 0.48, y: 0.84, a: 0.75, s: 21, p: 3.2 },
  { x: 0.9, y: 0.78, a: 0.8, s: 15, p: 0.2 },
] as const;

const ISO_START = -0.35;
const ISO_END = 2.15;
const ISO_STEP = 0.46;

function sample(nx: number, ny: number, t: number, aspect: number) {
  const x = nx * aspect;
  const y = ny;
  const wx = Math.sin(y * 3.1 + t * 0.18) * 0.07 + Math.sin(x * 1.6 - t * 0.11) * 0.05;
  const wy = Math.cos(x * 2.4 - t * 0.14) * 0.06 + Math.sin(y * 1.9 + t * 0.09) * 0.04;
  const u = x + wx;
  const v = y + wy;
  let n =
    Math.sin(u * 3.6 + t * 0.12) * 0.42 +
    Math.sin(v * 2.9 - t * 0.09) * 0.36 +
    Math.sin((u * 0.85 + v * 1.25) * 2.4 + t * 0.07) * 0.28;

  for (const h of HILLS) {
    const hx = (h.x + Math.sin(t * 0.13 + h.p) * 0.035) * aspect;
    const hy = h.y + Math.cos(t * 0.1 + h.p * 1.3) * 0.03;
    const dx = u - hx;
    const dy = v - hy;
    n += h.a * Math.exp(-(dx * dx + dy * dy) * h.s);
  }
  return n;
}

function interp(iso: number, a: number, b: number, va: number, vb: number) {
  const t = (iso - va) / (vb - va || 1);
  return a + (b - a) * Math.min(1, Math.max(0, t));
}

function edge(
  which: number,
  x0: number,
  y0: number,
  x1: number,
  y1: number,
  v0: number,
  v1: number,
  v2: number,
  v3: number,
  iso: number,
): [number, number] {
  if (which === 0) return [interp(iso, x0, x1, v0, v1), y0];
  if (which === 1) return [x1, interp(iso, y0, y1, v1, v2)];
  if (which === 2) return [interp(iso, x0, x1, v3, v2), y1];
  return [x0, interp(iso, y0, y1, v0, v3)];
}

function fmt(n: number) {
  return n.toFixed(1);
}

/** Frozen isoline path matching the About banner field. */
export function topoPath(width: number, height: number, t = 4.2) {
  const gx = 88;
  const gy = 46;
  const aspect = width / height;
  const stride = gx + 1;
  const field = new Float32Array(stride * (gy + 1));

  for (let j = 0; j <= gy; j++) {
    const ny = j / gy;
    for (let i = 0; i <= gx; i++) {
      field[j * stride + i] = sample(i / gx, ny, t, aspect);
    }
  }

  const cw = width / gx;
  const ch = height / gy;
  const parts: string[] = [];

  const add = (a: [number, number], b: [number, number]) => {
    parts.push(`M${fmt(a[0])} ${fmt(a[1])}L${fmt(b[0])} ${fmt(b[1])}`);
  };

  for (let iso = ISO_START; iso <= ISO_END; iso += ISO_STEP) {
    for (let j = 0; j < gy; j++) {
      const y0 = j * ch;
      const y1 = y0 + ch;
      const row = j * stride;
      const next = row + stride;
      for (let i = 0; i < gx; i++) {
        const v0 = field[row + i]!;
        const v1 = field[row + i + 1]!;
        const v2 = field[next + i + 1]!;
        const v3 = field[next + i]!;
        const idx =
          (v0 >= iso ? 1 : 0) |
          (v1 >= iso ? 2 : 0) |
          (v2 >= iso ? 4 : 0) |
          (v3 >= iso ? 8 : 0);
        if (idx === 0 || idx === 15) continue;
        const x0 = i * cw;
        const x1 = x0 + cw;
        const pt = (which: number) =>
          edge(which, x0, y0, x1, y1, v0, v1, v2, v3, iso);
        switch (idx) {
          case 1:
          case 14:
            add(pt(3), pt(0));
            break;
          case 2:
          case 13:
            add(pt(0), pt(1));
            break;
          case 3:
          case 12:
            add(pt(3), pt(1));
            break;
          case 4:
          case 11:
            add(pt(1), pt(2));
            break;
          case 6:
          case 9:
            add(pt(0), pt(2));
            break;
          case 7:
          case 8:
            add(pt(3), pt(2));
            break;
          case 5:
            add(pt(3), pt(0));
            add(pt(1), pt(2));
            break;
          case 10:
            add(pt(3), pt(2));
            add(pt(0), pt(1));
            break;
        }
      }
    }
  }

  return parts.join("");
}

export function topoSvg(width: number, height: number) {
  const d = topoPath(width, height);
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none"><rect width="${width}" height="${height}" fill="#050505"/><path d="${d}" stroke="#9a9aa3" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" opacity="0.72"/></svg>`;
}
